...
export class RestaurantPage implements OnInit {
  @ViewChild("signshop",{read:ElementRef<Element>})signshop!:ElementRef<Element>;
  ...
  switch(shop:string){
    if(this.presentShop == shop){
       ...
       this.items.find((item)=>{
        item.firstElementChild?.innerHTML == shop//firstElementChild?访问第一个子元素，innerHTML 访问 label里面的文字，这就是获取每一个item的商店名
       })?.classList.remove("showshop")
       this.presentShop = "restaurant";
    }
   else {
    this.showProducts =this.productService.getShop("restaurant",shop);
    this.items.find((item)=>{item.firstElementChild?.innerHTML == this.presentShop})?.classList.remove("shwoshop")
    this.items.find((item)=>{
      item.firstElementChild?.innerHTML == shop//firstElementChild?访问第一个子元素，innerHTML 访问 label里面的文字，这就是获取每一个item的商店名
